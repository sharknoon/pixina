...
export default class Zoom {
  ...
  init() {
    ...
    const getPointInOriginal = (clientX: number, clientY: number) => {
      const mouseX = clientX - this.img.offsetLeft - (this.img.parentElement?.offsetLeft || 0)
      const mouseY = clientY - this.img.offsetTop - (this.img.parentElement?.offsetTop || 0)

      const imageWidthDelta = this.img.getBoundingClientRect().width - this.img.width
      const imageHeightDelta = this.img.getBoundingClientRect().height - this.img.height
